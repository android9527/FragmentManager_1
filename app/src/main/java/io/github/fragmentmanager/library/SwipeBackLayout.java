...
public class SwipeBackLayout extends FrameLayout {
    ...
    private FragmentActivity managerActivity;
    ...
    public View attachToFragment(Fragment fragment, View view) {
        if(!(fragment.getActivity() instanceof FragmentActivity))
            throw new RuntimeException("Main Activity must be FragmentActivity");

        managerActivity = (FragmentActivity) fragment.getActivity();
        if (view instanceof SwipeBackLayout) {
            View childView =  ((SwipeBackLayout)view).getChildAt(0);
            setBackground(fragment, childView);
        } else {
            setBackground(fragment, view);
        }
    ...
    private int getWindowBackground() {
        TypedArray a = managerActivity.getTheme().obtainStyledAttributes(new int[]{
                android.R.attr.windowBackground
        });
        int background = a.getResourceId(0, 0);
        a.recycle();
    ...
    public void setBackground(Fragment fragment, View view) {
